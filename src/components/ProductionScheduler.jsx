...
function jobsToCSV(jobs) {
  const headers = [
    "id","name","client","line","start","end","due","color","status",
    "modules","crew","priority","progress",
    "drawings_ready","materials_ready","permits_ready","inspections_ready","notes",
  ];
  const rows = jobs.map((job) =>
    [
      job.id, job.name, job.client, job.line, job.start, job.end, job.due,
...
export default function ProductionScheduler() {
  ...
  function onGridMouseDown(e) {
    ...
    setSelectedId(null);
    setDrawing({ line, startX: getX(e.clientX), currentX: getX(e.clientX) });
  }
  ...
  function startDrag(e, jobId) {
    ...
    setDragging({ jobId, startX: getX(e.clientX), origStart: job.start, origEnd: job.end });
  }
  ...
  useEffect(() => {
    function onMove(e) {
      ...
      if (drawing) {
        setDrawing((c) => ({ ...c, currentX: getX(e.clientX) }));
      }
    }
    function onUp() {
