...
export default function OTDDashboard() {
  ...
  if (isLoading) return <div style={{ padding: 24, color: '#6b7280' }}>Loading OTD dashboard…</div>;
  if (error) return <div style={{ padding: 24, color: '#b91c1c' }}>Failed to load OTD dashboard.</div>;
  if (!data) return null;
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      ...
      <div style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: 10 }}>
        ...
        {lateJobs.length === 0 && <div style={{ color: '#6b7280' }}>No late completed jobs.</div>}
        {lateJobs.map((j) => (
